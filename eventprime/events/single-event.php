...
if (isset($args->event->em_id) && post_password_required($args->event->em_id)) {
        ...
        <div class="col-md-8 mb-4 mb-md-0">
            ...
            <div class="is-event-info">
                <div class="d-flex gap-md-4 gap-2 mb-2 flex-column flex-md-row">
                    ...
                    <?php
                    $max_capacity_prefix = get_field('pc_maximum_capacity_prefix');
                    $max_capacity = get_field('pc_maximum_capacity');
                    $max_capacity_suffix = get_field('pc_maximum_capacity_suffix');
                    ...
                    if ($max_capacity) { ?>
                        ...
                        <div class="is-event-info__capacity">
                            ...
                            <span><?php echo esc_html($max_capacity_prefix) ?> <?php echo esc_html($max_capacity) ?> <?php echo esc_html($max_capacity_suffix) ?></span>
                        </div>
                    <?php } ?>

                    <?php
                    $show_time = get_field('pc_show_time');
                    if ($show_time) {
    ...
    <!-- Informatio Box -->
    <?php
    $show_information_box = get_field('show_information_box');
    if ($show_information_box):
        $information_box_title = get_field('information_box_title');
        $information_box_content = get_field('information_box_content');
    ?>
        <div class="row" id="event-information-box">
            <div class="col-md-12">
                <div class="pc-event-information-box">
                    <h4 class="pc-simplistic-headline"><?php echo esc_html($information_box_title); ?></h4>
                    <?php echo wp_kses_post($information_box_content); ?>
                </div>
            </div>
        </div>
    <?php endif; ?>
    ...
    <div class="row mb-5" id="event-booking-form">
        <div class="col-md-12">
            <div class="pc-booking-form mx-auto">
                <?php $booking_form = get_field('pc_booking_form'); ?>
                <?php
                if ($booking_form && !is_array($booking_form)) {
                    echo do_shortcode('[contact-form-7 id="' . $booking_form . '" title="Booking Form"]');
                }
                ?>
            </div>
        </div>
    ...
    <!-- Event Section -->
    <?php echo do_shortcode('[get_section_events]'); ?>
